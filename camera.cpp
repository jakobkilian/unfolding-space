...
Author: Jakob Kilian
Date: 26.09.19
Info: Handles the pmdtec pico flexx camera ( Init, Setup, receiveing frames, etc.) and the image processing
*/
...
int minObjSizeThresh = 90;  //the min number of pixels, an object must have (smaller objects might be noise
  float distThresh = 1.5;   //The depth of viewing range. Objects with bigger distance to camera are ignored. Value can be changed by poti.
  ...
  //_______________________MAIN PART OF IMAGE PROCESSING_______________________
  ...
  {
    int histo[9][256];                      //historgram, needed to find closest obj and create tiles
    lastNewData=millis();                   //timestamp when new frame arrives (to check if a crash of libroyale occured)
    getPauseDur();                          //calcucalte how long the code waited for a new "onNewData"
    if (potiAv){
      distThresh=globalPotiVal/100/3;       //calculate current distThresh (depth of viewing range) from potiVal
    }
